...
var generateDynamicJSONData = function () {
  var users = Array.from({ length: 10 }).map(function (_, index) {
    return {
  ...
  var uploads = users
    .map(function (user) {
      return Array.from({ length: 10 }).map(function (_, ui) {
        return {
          user: user,
          id: user.id * 10 + ui,
          date: faker_1.faker.date.past({ years: 1 }),
          workouts: Array.from({ length: 2 }).map(function (_, wi) {
            return {
    ...
    .flat();
  var workouts = uploads
    .map(function (upload) {
      return upload.workouts;
    })
    .flat();
  return JSON.stringify(
    { users: users, uploads: uploads, workouts: workouts },
    null,
    2
